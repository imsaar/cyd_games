...
static const MpShellConfig kCfg = { "dotsboxes", "Dots & Boxes", /*show_cpu_button=*/true, /*show_idle_peers=*/false };

// ── Mode selection ──

void DotsBoxes::mode_cpu_cb(lv_event_t*) {
    if (!s_self) return;
...
void DotsBoxes::mode_local_cb(lv_event_t*) {
    if (!s_self) return;
...
void DotsBoxes::mode_online_cb(lv_event_t*) {
    if (!s_self) return;
...
void DotsBoxes::on_host_ready(const Peer& peer) {
    if (!s_self) return;
...
void DotsBoxes::on_guest_ready(const Peer& peer) {
    if (!s_self) return;
    s_self->mode_ = MODE_NETWORK;
    s_self->my_player_ = P2;
...
void DotsBoxes::on_game_data(const char* json) {
