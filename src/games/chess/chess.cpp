...
static const MpShellConfig kCfg = { "chess", "Chess", /*show_cpu_button=*/true, /*show_idle_peers=*/false };
...
void Chess::mode_cpu_cb(lv_event_t*) {
    if (!s_self) return;
...
void Chess::mode_local_cb(lv_event_t*) {
    if (!s_self) return;
...
void Chess::mode_online_cb(lv_event_t*) {
    if (!s_self) return;
...
void Chess::on_host_ready(const Peer& peer) {
    if (!s_self) return;
...
void Chess::on_guest_ready(const Peer& peer) {
    if (!s_self) return;
    s_self->mode_ = MODE_NETWORK;
...
void Chess::on_game_data(const char* json) {
...
void Chess::show_result(const char* text, bool is_win) {
    ...
    lv_obj_t* again_btn = ui_create_btn(ov, "Play Again", 120, 36);
    lv_obj_align(again_btn, LV_ALIGN_BOTTOM_MID, -65, -15);
    lv_obj_add_event_cb(again_btn, [](lv_event_t* e) { screen_manager_switch(screen_manager_current()); }, LV_EVENT_CLICKED, NULL);

    lv_obj_t* btn = ui_create_btn(ov, "Menu", 90, 36);
...
void Chess::destroy() {
    ...
    s_self = nullptr; screen_ = nullptr; lbl_status_ = nullptr;
}
