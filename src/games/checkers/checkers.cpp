...
static const MpShellConfig kCfg = { "checkers", "Checkers", /*show_cpu_button=*/true, /*show_idle_peers=*/false };
static int board_ox_ = 0;
static int board_oy_ = 0;

// ── Mode selection ──

void Checkers::mode_cpu_cb(lv_event_t*) {
    if (!s_self) return;
...
void Checkers::mode_local_cb(lv_event_t*) {
    if (!s_self) return;
...
void Checkers::mode_online_cb(lv_event_t*) {
    if (!s_self) return;
...
void Checkers::on_host_ready(const Peer& peer) {
    if (!s_self) return;
...
void Checkers::on_guest_ready(const Peer& peer) {
    if (!s_self) return;
    s_self->mode_ = MODE_NETWORK;
...
void Checkers::on_game_data(const char* json) {
