...
void WhackMole::whack(int idx) {
    if (mole_up_[idx]) {
        if (mole_bomb_[idx]) {
            ...
            // Hit a mole — score based on speed
...
void WhackMole::spawn_mole() {
    ...
    for (int i = 0; i < HOLES; i++) {
        if (mole_up_[i]) active++;
    }
    if (active >= 3) return;  // Max 3 moles at once

    // Pick a random empty hole
    int empty[HOLES];
    int n = 0;
    for (int i = 0; i < HOLES; i++) {
        if (!mole_up_[i]) empty[n++] = i;
    }
    if (n == 0) return;
    ...
    // 20% chance of bomb after round 8
    mole_bomb_[idx] = (round_ > 8 && random(0, 5) == 0);

    if (mole_bomb_[idx]) {
        lv_label_set_text(hole_lbls_[idx], LV_SYMBOL_WARNING);
        lv_obj_set_style_text_color(hole_lbls_[idx], UI_COLOR_ACCENT, 0);
        lv_obj_set_style_bg_color(hole_btns_[idx], lv_color_hex(0x4a1a1a), 0);
    } else {
        lv_label_set_text(hole_lbls_[idx], LV_SYMBOL_UP);
        lv_obj_set_style_text_color(hole_lbls_[idx], UI_COLOR_SUCCESS, 0);
        lv_obj_set_style_bg_color(hole_btns_[idx], lv_color_hex(0x1a3a1a), 0);
    }
}
...
void WhackMole::update() {
    ...
    // Hide expired moles
    for (int i = 0; i < HOLES; i++) {
        if (mole_up_[i] && (now - mole_time_[i] > (uint32_t)mole_duration_)) {
            if (!mole_bomb_[i]) misses_++;  // Missed a mole (not a bomb)
            hide_mole(i);
        }
    }
